...
func EnCodeSSTableMeta(meta *SSTableMeta) []byte {
	...
	bytes := make([]byte, 0, config.SS_TABLE_META_SIZE)

	bytes = append(bytes, Uint64ToBytes(meta.Size)...)
	// block start and end id
	bytes = append(bytes, StrToBytes(meta.BeginId, config.DATA_BLOCK_ID_SIZE)...)
	bytes = append(bytes, StrToBytes(meta.EndId, config.DATA_BLOCK_ID_SIZE)...)
...
func EnCodeSSTableIndex(index *SSTableIndex) []byte {
	...
	for k, v := range index.Index {
		bytes = append(bytes, StrToBytes(k, config.DATA_BLOCK_ID_SIZE)...)
...
func EnCodeSSTableNodeMeta(meta *SSNodeMeta) []byte {
	...
	bytes := make([]byte, 0, config.SS_NODE_META_SIZE)

	bytes = append(bytes, StrToBytes(meta.Id, config.DATA_BLOCK_ID_SIZE)...)
